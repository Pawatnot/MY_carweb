...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 💡 จุดที่แก้ไข: ดึงข้อความแจ้งเตือนจากหลังบ้านมาแสดง
      if (error.response && error.response.data && error.response.data.message) {
        alert(error.response.data.message); // จะโชว์ข้อความ "บัญชีของคุณยังไม่ได้รับการอนุมัติ..."
      } else {
        alert("อีเมลหรือรหัสผ่านไม่ถูกต้อง กรุณาลองใหม่"); // เผื่อกรณีเซิร์ฟเวอร์ล่มหรือพัง
      }
    }
  };
